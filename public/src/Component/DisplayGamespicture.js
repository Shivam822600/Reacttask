...
{ const[list,setList]=useState()
  ...
  var result=await postData("gamepicture/deletedata",body)
  if(result)
  {
   swal({
...
else
{
 swal({
  ...
  const handleClick=async()=>{
    ...
    if(isBlank(gameid))
    {error=true
    msg+="<font color='#b2bec3'><b>game Should not be blank...</b></font><br>";
    }
    msg+="</div>"
    if(error)
    {
   ...
   var result=await postData('gamepicture/editdata',body)
...
const handleClickSaveIcon=async()=>{
  ...
  var result=await postDataAndImage('gamepicture/editicon',formData,config)
  if(result)
  {
   swal({
...
var result=await getData("gamepicture/displayall")
setList(result)
