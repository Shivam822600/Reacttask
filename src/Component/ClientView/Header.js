...
export default function Header(props) {
  ...
  function calculation(a,b){
    var price=b.discount>0?b.discount*b.qtydemand*b.duration:b.rentamt*b.qtydemand*b.duration;
    return a+price
  }
  function calculationsaving(a,b){
    var price=(b.rentamt-b.discount)*b.qtydemand*b.duration;
    return a+price
  }
  ...
  return values.map((item)=>{
     ...
     <div style={{ fontSize: 10, fontWeight: "bold", padding:2 }}>
              ...
              <div style={{ fontSize: 10, padding: 2 }}>
                ...
                <b>&#8377; {item.rentamt - item.discount}</b>
              </div>
              <div style={{ fontSize: 10, padding: 2 }}>
               <b>{item.qtydemand} x {item.discount>0?
      (<><span>&#8377; {item.discount}</span> x <span>{item.duration} Day(s)</span></>):(<span>&#8377; {item.rate}</span>)}
   </b>
