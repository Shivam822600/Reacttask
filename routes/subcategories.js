...
pool.query("insert into subcategories (categoryid,subcategoryname,description,discount,price,stock,rented,rentamt,icon,ad,adstatus)values(?,?,?,?,?,?,?,?,?,?,?)",
[req.body.categoryid,req.body.subcategoryname,req.body.description,req.body.discount,req.body.price,req.body.stock,req.body.rented,req.body.rentamt,req.files[0].originalname,req.files[1].originalname,req.body.adstatus],function(error,result){
...
router.post('/editicon',upload.single('icon'), function(req, res, next) {
    ...
    router.post('/editad',upload.single('ad'), function(req, res, next) {
        ...
        router.post('/editcategorydata', function(req, res, next) {
            pool.query("update subcategories set categoryid=?,subcategoryname=?,description=?,discount=?,price=?,stock=?,rented=?,rentamt=?,adstatus=? where subcategoryid=?",[req.body.categoryid,req.body.subcategoryname,req.body.description,req.body.discount,req.body.price,req.body.stock,req.body.rented,req.body.rentamt,req.body.adstatus,req.body.subcategoryid],function(error,result){
            console.log(result)
            if(error)
            {console.log(error)
                res.status(500).json({result:false})
